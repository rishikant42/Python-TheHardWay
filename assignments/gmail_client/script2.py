...
from helpers import Database, GmailInbox


SCOPE = 'https://www.googleapis.com/auth/gmail.modify'
USER_ID = 'me'
API_CREDENTIALS = 'credentials.json'
TOKEN = 'token_modify.json'
...
def create_sql_fetch_query(rules_set, overall_predicate):
    ...
    for rule in rules_set:
        ...
        # In case of date, value will be no. of days. Convert it into date before to pass in sql-query
        if field_name == 'date':
            if 'less_than' in predicate:
                value = date.today() - timedelta(int(value))
            else:
                value = date.today() + timedelta(int(value))
...
def take_action(action, msg_ids, label=None):
    ...
    """
    inbox = GmailInbox(SCOPE, API_CREDENTIALS, TOKEN)
