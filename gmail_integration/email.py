...
mssg_list = unread_msgs['messages'][:5]

msg_list.extend(mssg_list)

#while 'nextPageToken' in response:
## for i in range(5):
##     page_token = unread_msgs['nextPageToken']
##     unread_msgs = GMAIL.users().messages().list(userId='me',labelIds=[label_id_one, ], pageToken=page_token).execute()
##     msg_list.extend(unread_msgs['messages'])
...
for mssg in msg_list:
	...
	for h in headr: # getting the Subject
		if h['name'] == 'Subject':
			msg_subject = h['value']
	...
	temp_dict['Snippet'] = message['snippet'].encode('utf-8')# fetching message snippet
	...
	try:
		...
		temp_dict['Message_body'] = mssg_body.encode('utf-8')
...
with open('CSV_NAME2.csv', 'w') as csvfile: 
